...
export const normalizeEvent = (eventData: TrackRequest): NormalizedEvent => {
  ...
  const normalizedUserData: NormalizedUserData = {
    ...
    country: userData?.country ? userData.country.toLowerCase().trim() : (geoData?.country?.code ? geoData.country.code.toLowerCase() : null),
    state: userData?.state ? userData.state.toLowerCase().trim() : (geoData?.region?.code ? geoData.region.code.toLowerCase() : null),
    city: userData?.city ? userData.city.toLowerCase().trim() : (geoData?.city ? geoData.city.toLowerCase() : null),
    zip: userData?.zip ? userData.zip.replace(/\D/g, '') : (geoData?.postal || null),
